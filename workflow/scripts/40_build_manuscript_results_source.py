...
- Missing/duplicated/unexpected rows cause failure.
"""
...
OUT = ROOT / "results/publication/MANUSCRIPT_RESULTS_SOURCE.tsv"
RUN_INFO = ROOT / "results/publication/MANUSCRIPT_RESULTS_SOURCE_run_info.tsv"
...
for name, path in SOURCES.items():
    if not path.exists():
        fail(f"Required source missing [{name}]: {relative(path)}")
...
add_deg_rows(
    ...
    "Spatial-design result; do not use as cross-species control comparison",
)
...
coverage_specs = [
    (
        "Fusarium",
        "Induced",
        "F4_COG_FUS_INDUCED_COVERAGE",
        "1213 of 1550 genes",
    ),
    (
        "Fusarium",
        "Repressed",
        "F4_COG_FUS_REPRESSED_COVERAGE",
        "1219 of 1423 genes",
    ),
    (
        "Ophiostoma",
        "Induced",
        "F4_COG_OPH_INDUCED_COVERAGE",
        "2220 of 2587 genes",
    ),
    (
        "Ophiostoma",
        "Repressed",
        "F4_COG_OPH_REPRESSED_COVERAGE",
        "1859 of 2351 genes",
    ),
]

for organism, direction, result_id, _ in coverage_specs:
...
fus_cog_value = (
    "G induced; "
    + ",".join(sorted(expected_cog_sets[("Fusarium", "Repressed")]))
    + " repressed"
)

oph_cog_value = (
    ",".join(sorted(expected_cog_sets[("Ophiostoma", "Induced")]))
    + " induced; "
    + ",".join(sorted(expected_cog_sets[("Ophiostoma", "Repressed")]))
    + " repressed"
)
...
if len(interaction_rows) != 23:
    fail(
        "Expected 23 significant species × COG interaction rows; "
...
extra_fus = one(extra, "Fusarium extracellular summary", species="Fusarium")
extra_oph = one(extra, "Ophiostoma extracellular summary", species="Ophiostoma")
...
for row, species, context, prefix in [
    ...
    add(
        ...
        "genes",
        "extracellular",
        "Subset of significant CAZymes and SignalP-positive genes",
    )


cazy = read_tsv(SOURCES["cazyme_audit"])

cazy_fus = one(cazy, "Fusarium CAZyme audit", species="Fusarium")
cazy_oph = one(cazy, "Ophiostoma CAZyme audit", species="Ophiostoma")
...
for row, species, context, prefix in [
    ...
    if row["status"] != "PASS":
        fail(f"{species} CAZyme audit status is not PASS")
...
if classified_total != reported_classified:
    fail(
        "Spatial-class sum does not equal run_info classified_gene_count: "
...
result_ids = [row["result_id"] for row in rows_out]

if len(result_ids) != len(set(result_ids)):
    fail("Duplicate manuscript result_id detected")


# Cross-table invariants.
if int(fus["significant_total"]) != int(extra_fus["significant_de_genes"]):
    fail("Fusarium DEG count disagrees with extracellular summary")

if int(oph_control["significant_total"]) != int(extra_oph["significant_de_genes"]):
    fail("Ophiostoma DEG count disagrees with extracellular summary")

if int(cazy_fus["unique_gene_count"]) != int(
    extra_fus["significant_high_confidence_cazymes"]
):
    fail("Fusarium CAZyme count disagrees across canonical tables")

if int(cazy_oph["unique_gene_count"]) != int(
    extra_oph["significant_high_confidence_cazymes"]
):
    fail("Ophiostoma CAZyme count disagrees across canonical tables")

if int(cazy_fus["secreted_genes"]) != int(
    extra_fus["significant_secreted_cazymes"]
):
    fail("Fusarium secreted CAZyme count disagrees across canonical tables")

if int(cazy_oph["secreted_genes"]) != int(
    extra_oph["significant_secreted_cazymes"]
):
    fail("Ophiostoma secreted CAZyme count disagrees across canonical tables")
...
run_info = [
    ...
    (
        ...
        (
            "Ophiostoma: interaction=reaction zone; "
            "self=non-contact region; onu=control colony"
        ),
    ),
    (
...
with RUN_INFO.open("w", newline="", encoding="utf-8") as handle:
    writer = csv.writer(
        handle,
        delimiter="\t",
        lineterminator="\n",
...
print()
print("PASS: canonical values extracted and cross-table invariants validated.")
