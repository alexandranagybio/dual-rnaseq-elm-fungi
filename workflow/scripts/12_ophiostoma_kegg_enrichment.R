...
# 12_ophiostoma_kegg_enrichment.R
#
# Directional KEGG over-representation analysis for rebuilt Ophiostoma
# novo-ulmi gene-level DESeq2 results.
#
...
EXPECTED_KEGG_GENES <- 1149L
EXPECTED_GENE_PATHWAY_PAIRS <- 2754L
EXPECTED_PATHWAYS <- 111L
...
output_dir <- file.path(
  "results", "ophiostoma", "kegg_enrichment"
)
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

gene_map_file <- file.path(
  annotation_dir, "ophiostoma_gene_annotation_map.tsv"
)
term2gene_file <- file.path(
  kegg_annotation_dir, "ophiostoma_kegg_term2gene.tsv"
)
metadata_file <- file.path(
  kegg_annotation_dir, "ophiostoma_kegg_pathway_metadata.tsv"
)

contrast_files <- c(
  interaction_vs_self = file.path(
    annotation_dir, "interaction_vs_self_all_genes.annotated.tsv"
  ),
  onu_vs_self = file.path(
    annotation_dir, "onu_vs_self_all_genes.annotated.tsv"
  ),
  interaction_vs_onu = file.path(
    annotation_dir, "interaction_vs_onu_all_genes.annotated.tsv"
  )
)
...
unmapped <- setdiff(unique(term2gene_raw$gene_id), gene_map$gene_id)
if (length(unmapped) > 0L) {
  stop_with(
    "TERM2GENE genes absent from annotation map: ",
...
metadata_unique <- unique(
  pathway_metadata[, c("pathway", "pathway_class", "pathway_type")]
)
if (anyDuplicated(metadata_unique$pathway)) {
  stop_with(
    "A pathway is associated with multiple class/type combinations."
  )
}
if (!setequal(
  unique(term2gene_raw$pathway),
  metadata_unique$pathway
)) {
...
term2name <- metadata_unique[, c("pathway", "pathway")]
...
write.table(
  ...
  file = file.path(
    output_dir,
    "12_ophiostoma_kegg_enrichment.validation.tsv"
  ),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
...
writeLines(
  ...
  con = file.path(
    output_dir,
    "12_ophiostoma_kegg_enrichment.sessionInfo.txt"
  )
)

message("")
message("KEGG enrichment completed successfully.")
